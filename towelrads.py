...
class towelrads_message:
    ...
    # The operation the radiator should now run in.
    operation = towelrads_mode.COMFORT
...
def towelrads_message_to_ir_signal(message):
    pass

# Converts an IR signal recieved from a UFO-R11 IR blaster to a towelrads message.
def ir_signal_to_towelrads_message(message):
    pass
...
def equal_with_epsilon(value, target, epsilon = timing_epsilon):
...
def decode_message(ir, as_binary):
    ...
    for index in range(0, len(ir)):
        ...
        if (equal_with_epsilon(duration_us, header_timings[0]) and equal_with_epsilon(ir[index + 1], header_timings[1]) and index < len(ir) - 1):
            ignore_next = True
            continue
        ...
        # Else decode as on/off signals
            ...
            if (not signal_high):
                ...
                # Single down = 0
                if (equal_with_epsilon(duration_us, low_time)): 
                    #print(str(duration_us)+" = 0")
                    signal_bit = 0
                # Double down = 1 
                elif (equal_with_epsilon(duration_us, low_time * 2)): 
                    #print(str(duration_us)+" = 1")
                    signal_bit = 1
                # Gap before repeat
                elif (equal_with_epsilon(duration_us, repeat_interval_time, high_time)): 
                ...
                if (not equal_with_epsilon(duration_us, high_time)): 
